...
SCRIPTS = (
    REPOSITORY
    / "plugins/versionkeeping/skills/checkpointing-and-publishing-git-work/scripts"
)
sys.path.insert(0, str(SCRIPTS))

from git_publication.core import (  # noqa: E402
    AbsentTarget,
    CreationBase,
    PresentTarget,
    PublicationRequest,
...
class CorePlanningTests(unittest.TestCase):
    def test_present_target_requires_a_sha(self):
        with self.assertRaises(ValueError):
            PresentTarget(
                sha=None, outgoing_shas=(), target_only_shas=(), is_ancestor=False
            )
    ...
    def test_ready_fast_forward_uses_immutable_source_and_exact_lease(self):
        ...
        self.assertEqual(result["push"]["source_sha"], B)
        self.assertEqual(result["push"]["ref"], "refs/heads/topic")
        self.assertEqual(result["push"]["refspec"], f"{B}:refs/heads/topic")
        self.assertEqual(
            result["push"]["lease"], f"--force-with-lease=refs/heads/topic:{A}"
        )
    ...
    def test_unowned_outgoing_commit_blocks(self):
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertEqual(
            result["reasons"][0]["code"], "OUTGOING_COMMITS_NOT_OWNED_OR_ADOPTED"
        )
    ...
    def test_absent_target_requires_creation_authorization(self):
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertEqual(
            result["reasons"][0]["code"], "TARGET_ABSENT_CREATION_NOT_ALLOWED"
        )
    ...
    def test_absent_advertised_start_uses_expected_absent_lease(self):
        ...
        self.assertEqual(result["status"], "ready")
        self.assertEqual(
            result["push"]["lease"], "--force-with-lease=refs/heads/topic:"
        )
    ...
    def test_creation_base_requires_every_base_to_start_commit_adopted(self):
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertEqual(
            result["reasons"][-1]["code"], "CREATION_BASE_TO_START_COMMITS_NOT_ADOPTED"
        )
