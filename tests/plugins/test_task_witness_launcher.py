...
class TaskWitnessLauncherTests(unittest.TestCase):
    ...
    def launch(
        ...
        *,
        ...
        driver = self.root / "fixture_launcher_driver.py"
        driver.write_text(
            "import importlib.util\n"
            "import sys\n"
            "from pathlib import Path\n"
            "specification = importlib.util.spec_from_file_location(\n"
            "    'task_witness_launcher_fixture', sys.argv[1]\n"
            ")\n"
    ...
    def test_protected_nodes_reject_group_or_other_permissions(self) -> None:
        for target in (
            ...
            self.install / "generations",
            self.generation,
            self.generation / "canonical.py",
        ):
            ...
            with self.subTest(target=target.name):
                result = self.launch()
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("current-user private", result.stderr)
    ...
    def test_registered_validator_future_annotations_are_source_scoped(self) -> None:
        ...
        helper.write_text("def helper(value: 1 / 0) -> 1 / 0:\n    return value\n")
        self.write_inputs(modules=[("validator", self.validator), ("helper", helper)])
