...
# fmt: off
CLIENT_SOURCE_GENERATION_SHA256 = "ac535d823a48a9607dccf880261f490ee3fb551a58a476d8710084840c812ddd"
# fmt: on
...
def _validate_compatibility_policy(raw: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        or not _same_json(contracts, CLIENT_CONTRACTS)
    ):
    ...
    if source["mode"] in {"harness_snapshot", "publisher_channel"}:
        ...
        if (
            ...
            or not _text(details["revision"])
            or not _digest(details["subtree_sha256"])
        ):
...
def _validate_receipt_source(value: object) -> dict[str, Any]:
    ...
    if not _token(lineage["lineage_id"]) or not _nonnegative_integer(
        lineage["sequence"]
    ):
        raise ValueError("deployment source lineage is invalid")
    for key in (
        "source_selection_sha256",
        "source_selection_content_sha256",
        "manager_binding_sha256",
...
def _validate_receipt_contracts(value: object) -> None:
    if not _same_json(value, RECEIPT_CONTRACTS):
    ...
    *,
    ...
    def validate_receipt_semantics(
        ...
        if policy is None:
            ...
            policy = _validate_compatibility_policy(policy_raw)
        ...
        source = _validate_receipt_source(value["source"])
    ...
    def validate_receipt_header(
        ...
        if (
            set(value) != deployment_receipt_keys
            or type(value["schema_version"]) is not int
            or value["schema_version"] != 1
            or value["contract"] != DEPLOYMENT_RECEIPT_CONTRACT
        ...
        _validate_receipt_contracts(value["contracts"])
    ...
    while True:
        ...
        validate_receipt_header(prior, "prior deployment receipt")
        if (
        ...
        if control_maintenance:
            ...
            for index, (role, relative, installed_mode, limit) in enumerate(
                ...
                if (
                    ...
                    raise ValueError("active rollback control preimage mismatch")
                if role == "policy":
                    prior_policy = _validate_compatibility_policy(staged_raw)
...
def _load_installation(
    ...
    source = _validate_receipt_source(receipt["source"])
    _validate_receipt_runtime_closure(receipt["runtime_closure"])
    _validate_receipt_contracts(receipt["contracts"])
...
def _validate_activation_transaction(
    ...
    if prior["state"] == "absent":
        ...
        if (
            ...
            or parser["deployment_receipt_contract"] != DEPLOYMENT_RECEIPT_CONTRACT
...
def _capture_control_maintenance_stage(
    ...
    file_parts = {("stage.json",), *relative_parts}
    directory_parts = {
    ...
    expected_children: dict[tuple[str, ...], set[str]] = {(): {"stage.json"}}
    ...
    candidate_policy = _validate_compatibility_policy(candidate_policy_raw)
    prior_policy = _validate_compatibility_policy(prior_policy_raw)
    ...
    if (
        ...
        or candidate_authorization["purpose"] != "complete-control-set-maintenance"
...
def _receipt_activation_smoke_authority_projection(
    ...
    if authorization["purpose"] == "complete-control-set-maintenance":
