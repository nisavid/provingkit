...
DEPLOYMENT_RECEIPT_CONTRACT = "task-witness-deployment-receipt-v1"
...
class CandidateSource:
    ...
    source_authority: str
    channel: str
    manager_trust_class: str
    lineage: Mapping[str, Any]
    manager_receipt_sha256: str
    source_selection_content_sha256: str
    source_selection_sha256: str
    manager_binding_content_sha256: str
    manager_binding_sha256: str
...
class ActivationRequest:
    ...
    deployment: FirstInstallRequest | DeploymentRequest | RollbackToRequest
...
def _parse_plugin_manifest(raw: bytes, harness: str) -> PluginManifest:
    label = f"{harness} plugin manifest"
    value = _parse_strict_json(raw, label)
    common = {
        "name",
        "version",
    ...
    if harness == "claude":
        value = _exact(value, common | {"displayName"}, label)
        _text(value["displayName"], f"{label}.displayName")
    elif harness == "codex":
        required = common | {"interface"}
        allowed = required | {"skills"}
        if not required.issubset(value) or not set(value).issubset(allowed):
            raise DeploymentError(f"{label} schema drift")
        if "skills" in value and value["skills"] != "./skills/":
            raise DeploymentError(f"{label}.skills contract mismatch")
        ...
        interface_allowed = interface_required | {"defaultPrompt", "brandColor"}
        if (
            not isinstance(interface, dict)
            or not interface_required.issubset(interface)
            or not set(interface).issubset(interface_allowed)
        ):
            raise DeploymentError(f"{label}.interface schema drift")
        for key in interface_required - {"capabilities"}:
            _text(interface[key], f"{label}.interface.{key}")
        _string_inventory(
            interface["capabilities"],
            f"{label}.interface.capabilities",
        )
        if "defaultPrompt" in interface:
            _string_inventory(
                interface["defaultPrompt"],
                f"{label}.interface.defaultPrompt",
            )
        if "brandColor" in interface:
            _text(interface["brandColor"], f"{label}.interface.brandColor")
    else:
        raise DeploymentError("plugin manifest harness is unsupported")
    _text(value["description"], f"{label}.description")
    _text(value["homepage"], f"{label}.homepage")
    _text(value["license"], f"{label}.license")
    _string_inventory(value["keywords"], f"{label}.keywords", allow_empty=True)
    return PluginManifest(
        harness,
        _token(value["name"], f"{label}.name"),
        _text(value["version"], f"{label}.version"),
        _freeze(_manifest_author(value["author"], f"{label}.author")),
        _text(value["repository"], f"{label}.repository"),
...
def _parse_compatibility_policy(raw: bytes) -> CompatibilityPolicy:
    ...
    if mode in {"harness_snapshot", "publisher_channel"}:
        ...
        normalized_details = {
            ...
            "subtree_sha256": _sha256(
                details["subtree_sha256"],
                "compatibility policy subtree digest",
            ),
        }
    else:
...
def _validate_first_install_authorization(
    ...
    *,
    ...
    expected_digests = {
        ...
        "manager_receipt_sha256": _sha256(
            manager_receipt_sha256,
            "expected manager-receipt digest",
        ),
    }
    ...
    return FirstInstallAuthorization(
        ...
        digests["manager_receipt_sha256"],
        _content_sha256(value, label),
    )
...
def _validate_deployment_authorization(
    ...
    *,
    ...
    label = (
        "routine deployment authorization"
        if expected_purpose == "routine-compatible-forward"
        else "control-set maintenance authorization"
    )
    value = _exact(
        _parse_canonical_json(raw, label),
        {
...
def _bind_candidate_source(
    ...
    """Construct one composite identity by comparing only contract-owned fields."""
    ...
    codex_raw = _candidate_file(
        snapshot,
        CODEX_MANIFEST_NAME,
        "the Codex manifest",
    )
    provider = _parse_provider(provider_raw) if provider_raw is not None else None
    claude = _parse_plugin_manifest(claude_raw, "claude")
    codex = _parse_plugin_manifest(codex_raw, "codex")
    selection = _parse_source_selection(source_selection_raw)
    binding = _parse_manager_binding(manager_binding_raw, manager_receipt_raw)
    if selection.mode != "harness_snapshot":
        raise DeploymentError(
            "candidate source manager binding requires harness_snapshot"
        )
    details = selection.details
    claims = binding.claims
    if (
        claude.name != codex.name
        or claude.name != claims["plugin_id"]
        or (provider is not None and provider.plugin_id != claude.name)
    ):
        raise DeploymentError("candidate source plugin ID cross-binding disagrees")
    if (
        claude.author != selection.manifest_author
        or codex.author != selection.manifest_author
        or (provider is not None and provider.publisher != selection.publisher_id)
    ):
        raise DeploymentError("candidate source publisher cross-binding disagrees")
    if (
        claude.repository != selection.repository_url
        or codex.repository != selection.repository_url
        or (provider is not None and provider.repository != selection.repository_url)
    ):
        raise DeploymentError("candidate source repository cross-binding disagrees")
    if (
        claude.version != codex.version
        or claude.version != selection.release_version
        or claude.version != claims["release_version"]
    ):
        raise DeploymentError(
    ...
    if (
        ...
        raise DeploymentError("candidate source manager cross-binding disagrees")
    return CandidateSource(
        ...
        details["manager_trust_class"],
        details["lineage"],
        binding.manager_receipt_sha256,
        selection.content_sha256,
        hashlib.sha256(source_selection_raw).hexdigest(),
        binding.content_sha256,
        hashlib.sha256(manager_binding_raw).hexdigest(),
...
def _classify_source_transition(
    *,
    ...
    same_lineage_position = (
        active_lineage["lineage_id"] == candidate_lineage["lineage_id"]
...
def _require_deployment_source_outcome(
    ...
    *,
    ...
    allowed = (
        {"compatible-forward", "approval-required"}
        if control_maintenance
        else {"compatible-forward"}
    )
    if classification.outcome not in allowed:
        raise DeploymentError(
...
def _plan_first_install(
    ...
    plan_unsigned = {
        ...
        "source": {
            ...
            "manager_binding_sha256": source.manager_binding_sha256,
            "manager_receipt_sha256": source.manager_receipt_sha256,
        },
...
def _first_install_deployment_receipt(
    ...
    unsigned = {
        ...
        "source": {
            ...
            "source_authority": source.source_authority,
            "channel": source.channel,
            "manager_trust_class": source.manager_trust_class,
            "lineage": _thaw(source.lineage),
            "source_selection_sha256": source.source_selection_sha256,
            "source_selection_content_sha256": (source.source_selection_content_sha256),
            "manager_binding_sha256": source.manager_binding_sha256,
            "manager_binding_content_sha256": (source.manager_binding_content_sha256),
            "manager_receipt_sha256": source.manager_receipt_sha256,
            "claude_manifest_sha256": source.claude_manifest_sha256,
            "codex_manifest_sha256": source.codex_manifest_sha256,
...
def _materialize_first_install(
    ...
    authorization = _validate_first_install_authorization(
        ...
        manager_receipt_sha256=plan.source.manager_receipt_sha256,
    )
...
def _manual_rollback_stage_artifacts(
    ...
    *,
    ...
    target_bindings = _manual_rollback_target_control_bindings(
        target.receipt_value
    )
    prior_bindings = {
...
def _materialize_routine_deployment(
    ...
    authorization_purpose = (
        ...
        if control_maintenance
        else "routine-compatible-forward"
    )
    ...
    if current != plan.precondition:
        raise DeploymentError("routine active precondition changed after staging")
    return StagedDeployment(
        plan,
        authorization,
        classification,
...
def _materialize_manual_rollback(
    ...
    if verified.raw != staged.stage_raw or tuple(
        (item.role, item.relative_path, item.raw, item.staged, item.installed)
        for item in verified.artifacts
    ) != tuple(
        (item.role, item.relative_path, item.raw, item.staged, item.installed)
        for item in staged.artifacts
    ):
        raise DeploymentError("manual rollback stage independent verify disagrees")
    return staged
...
def _validate_provider_source_and_modules(
    ...
    source = _active_receipt_source(deployment_value, canonical_root)
    source_declaration = (
...
def _staged_compatibility_policy(
    ...
    parsed = _parse_compatibility_policy(artifact.raw)
...
def _validate_routine_source_transition(
    ...
    *,
    ...
    active_source = _active_receipt_source(prior, canonical_root)
...
def _verify_deployment_stage(stage_receipt: Path) -> VerifiedDeploymentStage:
    ...
    if not any(
        (first_install_stage, routine_stage, control_stage, manual_stage)
    ):
    ...
    if routine_stage or control_stage or manual_stage:
...
def _validate_first_install_request(request: FirstInstallRequest) -> None:
    ...
    for label, raw in (
        ...
        ("manager binding", request.manager_binding_raw),
        ("manager receipt", request.manager_receipt_raw),
...
def _prepare_first_install_against_precondition(
    ...
    source = _bind_candidate_source(
        snapshot,
        request.source_selection_raw,
        request.manager_binding_raw,
        request.manager_receipt_raw,
    )
...
def _active_receipt_source(
    ...
    if source_mode != "harness_snapshot":
    ...
    return CandidateSource(
        ...
        _freeze(
            {
                "lineage_id": _token(lineage["lineage_id"], f"{label}.lineage_id"),
        ...
        _sha256(source["manager_receipt_sha256"], f"{label}.manager_receipt"),
        _sha256(
        ...
        _sha256(source["manager_binding_sha256"], f"{label}.manager_binding"),
        declaration_sha256,
        declaration_content_sha256,
...
def _validate_retained_receipt_header(
    ...
    receipt = _exact(value, _DEPLOYMENT_RECEIPT_KEYS, label)
    ...
    if (
        ...
        or receipt["contract"] != DEPLOYMENT_RECEIPT_CONTRACT
        or sequence < 1
    ...
    if _thaw(receipt["contracts"]) != RECEIPT_CONTRACTS:
...
def _validate_retained_receipt_policy(
    ...
    source = _active_receipt_source(receipt, canonical_root)
    controls = _exact(
        _thaw(receipt["control_set"]),
...
def _validate_retained_control_preimage(
    ...
    for index, role in enumerate(CONTROL_PREIMAGE_ROLES):
        ...
        if (
            ...
            raise DeploymentError(f"{label} bytes disagree")
        if role == "policy":
            prior_policy = _parse_compatibility_policy(raw)
...
def _validate_retained_receipt_chain_in_directory(
    ...
    *,
    ...
    while True:
        ...
        manual_rollback = (
            authorization["purpose"] == "manual-exact-target-rollback"
        )
        control_maintenance = (
            authorization["purpose"] == "complete-control-set-maintenance"
            or manual_rollback
        )
        if control_maintenance:
            rollback_keys.add("control_preimage")
    ...
    for edge in authority_edges:
        ...
        if target is not None and (
            target not in receipt_digests
            or target == edge.successor_receipt_sha256
        ):
...
def _capture_active_deployment_precondition(
    ...
    try:
        ...
        _content_sha256(receipt, "active deployment receipt")
        sequence = _nonnegative_integer(
            receipt["sequence"],
            "active deployment receipt sequence",
        )
        prior = receipt["prior_receipt_sha256"]
        if prior is not None:
            _sha256(prior, "active deployment prior receipt")
        lock_binding = {
            "path": str(canonical_root / "activation.lock"),
        ...
        if (
            ...
            or receipt["canonical_root"] != str(canonical_root)
            or receipt["effective_uid"] != os.geteuid()
            or receipt["activation_lock"] != lock_binding
            or receipt["process_profile"] != PROCESS_PROFILE
        ):
            raise DeploymentError("active deployment receipt binding disagrees")
...
def _resolve_manual_rollback_target_authority(
    ...
    if (
        hashlib.sha256(selected.prior_receipt_raw).hexdigest()
        != target_receipt_sha256
        or selected.selector_raws["deployment-alias"]
        != selected.prior_receipt_raw
    ...
    for role, binding in bindings.items():
        ...
        if (
            ...
            raise DeploymentError(
                f"manual rollback target {role} authority disagrees"
            )
...
def _prepare_rollback_to_against_precondition(
    ...
    unsigned = {
        ...
        "target_active_sha256": hashlib.sha256(
            target_authority.active_raw
        ).hexdigest(),
        "target_authority_sha256": target_authority.authority_sha256,
        "target_successor_receipt_sha256": (
            target_authority.successor_receipt_sha256
        ),
        ...
        "retained_receipt_names": sorted(
            precondition.retained_chain.receipt_names
        ),
...
def stage_first_install(
    ...
    if verified.raw != staged.stage_raw or tuple(
        ...
        raise DeploymentError("staged deployment independent verification disagrees")
    return staged
...
def stage_deployment(
    ...
    if verified.raw != staged.stage_raw or tuple(
        ...
        raise DeploymentError("routine stage independent verification disagrees")
    return staged
...
def _revalidate_locked_active_precondition(
    ...
    if _parse_compatibility_policy(policy_raw) != precondition.active_policy:
...
def _stage_bound_routine_source(
    ...
    binding = _parse_manager_binding(
        request.manager_binding_raw,
        request.manager_receipt_raw,
    )
    ...
    if selection.mode != "harness_snapshot":
        raise DeploymentError(
            "stage-bound routine source requires harness_snapshot"
        )
    if (
        ...
        or source.channel != details["channel"]
        or source.manager_trust_class != details["manager_trust_class"]
        or source.lineage != details["lineage"]
        ...
        or source.manager_binding_content_sha256 != binding.content_sha256
        or source.manager_binding_sha256
        != hashlib.sha256(request.manager_binding_raw).hexdigest()
        or source.manager_receipt_sha256 != binding.manager_receipt_sha256
        or binding.harness != details["harness"]
        or binding.manager != details["manager"]
        or binding.manager_receipt_sha256 != details["manager_receipt_sha256"]
        or claims["plugin_id"] != source.plugin_id
...
def _parse_control_maintenance_activation_journal(
    ...
    *,
    ...
    if terminal is not None:
        ...
        successful_outcome = (
            "manual-target-active" if manual else "candidate-active"
        )
        restored_outcome = (
            "manual-current-restored" if manual else "restored-prior"
        )
        target_rejection = (
...
def _parse_activation_journal(raw: bytes) -> _ActivationJournal:
    ...
    if (
        isinstance(preliminary, dict)
        and preliminary.get("transaction_class")
        == "manual-exact-target-rollback"
        and isinstance(preliminary.get("prior"), dict)
        and preliminary["prior"].get("state") == "active"
    ):
...
def _recovery_precondition_from_stage(
    ...
    if type(activation.deployment) in {DeploymentRequest, RollbackToRequest}:
        rollback_value = _parse_canonical_json(
            rollback.raw,
        ...
        if (
            ...
            or root != activation.deployment.canonical_root
            ...
            != activation.deployment.expected_active_receipt_sha256
        ):
...
def _reconstruct_control_maintenance_recovery_precondition(
    ...
    *,
    ...
    active_prior_class = (
        (
            type(activation.deployment) is DeploymentRequest
            and journal.value["transaction_class"] == "control-set-maintenance"
        )
        or (
            type(activation.deployment) is RollbackToRequest
            and journal.value["transaction_class"]
            == "manual-exact-target-rollback"
        )
    )
    if (
        not active_prior_class
        or journal.value["prior"]["state"] != "active"
    ):
    ...
    active_policy = _parse_compatibility_policy(control_raws["policy"])
    chain = _validate_retained_receipt_chain(
        receipt,
        receipt_raw,
        recorded.canonical_root,
...
def _audit_control_maintenance_live_state(
    ...
    *,
    ...
    if phase == "rollback-cleaning":
        ...
        phase == "terminal"
        and journal.value["terminal_result"]["outcome"]
        in {"restored-prior", "manual-current-restored"}
    ):
        removed_paths = {
            step.artifact.relative_path
            for step in cleanups
...
def _active_prior_control_recovery(
    ...
    return (
        (
            type(request.activation.deployment) is DeploymentRequest
            and journal.value["transaction_class"] == "control-set-maintenance"
        )
        or (
...
def _active_prior_recovery_intent_from_stage(
    ...
    if type(request.deployment) is DeploymentRequest:
        if not _rebind_routine_activation_from_stage(
            ...
            raise DeploymentError(
                "control maintenance recovery stage rebind disagrees"
            )
        intent, _ = _control_maintenance_activation_intent_from_stage(
            precondition,
            request,
...
def _recover_control_maintenance_transaction(
    ...
    if manual_rollback and expected.value["phase"] not in {
        ...
        raise DeploymentError(
            "manual rollback recovery phase is not implemented"
        )
    ...
    try:
        ...
        if (
            ...
            provisional_additive = (
                _ordered_control_maintenance_additive_artifacts(locked_verified)
            )
            provisional_replacements = (
                _ordered_control_maintenance_replacements(locked_verified)
            )
...
def _recover_control_maintenance_through_prior_controller(
    ...
    try:
        ...
        if type(deployment) is RollbackToRequest:
            rebound_deployment = module.RollbackToRequest(
            ...
            rebound_deployment = module.DeploymentRequest(
                ...
                manager_binding_raw=bytes(deployment.manager_binding_raw),
                manager_receipt_raw=bytes(deployment.manager_receipt_raw),
                runtime_qualification_raw=bytes(
                    deployment.runtime_qualification_raw
                ),
                maintenance_transaction_sha256=str(
                    deployment.maintenance_transaction_sha256
                ),
...
def recover_transaction(request: RecoveryRequest) -> TransactionResult:
    ...
    try:
        ...
        if type(request.activation.deployment) is DeploymentRequest:
            ...
            if (
                live.value["phase"] == "terminal"
                and live.value["terminal_result"]["outcome"]
                != "recovery-required"
            ):
                ...
                provisional_artifacts = _ordered_routine_activation_artifacts(
                    verified
                )
        ...
        if (
            ...
            provisional_artifacts = _ordered_activation_artifacts(
                provisional_verified
            )
...
def reconcile_transaction_result(
    ...
    try:
        ...
        if type(request.activation.deployment) is RollbackToRequest:
            if (
                expected.value["transaction_class"]
                != "manual-exact-target-rollback"
            ):
                raise DeploymentError(
            ...
            additive = _ordered_control_maintenance_additive_artifacts(
                locked_verified
            )
            replacements = _ordered_control_maintenance_replacements(
                locked_verified
            )
            cleanups = _ordered_control_maintenance_cleanup_steps(
                precondition,
