...
`rolecasting-dispatch-evidence-validator-v2` for
`rolecasting-dispatch-evidence-v2`. Its provider declares empty producer and
...
`rolecasting-bootstrap-dispatch-request-v2`. It freezes supplied facts and
...
- `manifest.json`, a `rolecasting-dispatch-evidence-v2` document;
- `plan.json`, a `rolecasting-dispatch-plan-v2` document; and
- one `model-<execution-id>.json` and `result-<execution-id>.json` per dispatch.
...
contracts; model-selection receipt; read-only authority; conditional user
...
Each `rolecasting-model-selection-receipt-v2` binds the target, selected model,
...
Each `rolecasting-execution-result-receipt-v2` binds the raw plan, dispatch, and
model; exact request; typed return, verification, and stop evidence; target,
...
`rolecasting-dispatch-projection-v2`. It contains the evidence and raw manifest
