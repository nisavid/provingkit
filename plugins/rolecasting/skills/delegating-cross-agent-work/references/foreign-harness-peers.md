...
Treat every follow-up, resume, retry, capacity recovery, or reclassification as
a new dispatch gate. Before sending continuation task data, revalidate the same
eligible task or session, account binding, exact target and selection, carried
judgment floor, sticky operator choice, data boundary, workspace, tools,
capacity, and external-action scope. Bind the authorized model-transition
decision to the payload. The task ID, stored model, or visibility in a shared
session database proves neither the original account route nor current
capacity.

For a Codex CLI continuation through a privately bound alternate account home,
explicitly set `CODEX_HOME` for the initial `codex exec` and every `codex exec
resume`, and pass the exact freshly proven model. Never use the ambient account
to recover affinity. If the continuation actuator cannot select and observe
the authorized binding before accepting data, fail closed. A newly created
task still needs creation authority and fresh route proof.
