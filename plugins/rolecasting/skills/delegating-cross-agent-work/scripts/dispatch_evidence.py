...
class _LocalEvidenceError(ValueError):
    """A dispatch bundle fails Rolecasting's local structural contract."""


class _LocalWitness:
    """Pure evidence primitives used when no optional Task Witness is injected.

    These checks cover shape, canonical encoding, and content identity only.
    They deliberately do not claim issuer or producer authority; those routes
    remain unavailable unless a caller supplies the optional witness adapter.
    """
    ...
    def __getattr__(self, name: str) -> Any:
        if name == "invoke_registered_validator":
