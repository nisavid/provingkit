...
def parse_file_link(line: str) -> FileLink | None:  # noqa: C901
    markdown_operation = MARKDOWN_OPERATION_FILE_LINK_RE.match(line)
    if markdown_operation:
        parsed = parse_github_url(_url(markdown_operation), file_link=True)
        if not parsed or _unsafe_path(
            markdown_operation.group("source_path"),
            markdown_operation.group("target_path"),
        ):
            return None
        return FileLink(
    ...
    if markdown:
        parsed = parse_github_url(_url(markdown), file_link=True)
        if not parsed or _unsafe_path(markdown.group("path")):
            return None
        return FileLink(
    ...
    if html_operation:
        ...
        if (
            ...
            return None
        parsed = parse_github_url(_url(html_operation), file_link=True)
        if not parsed or _unsafe_path(source_path, target_path):
            return None
        return FileLink(
    ...
    if raw_path != escape(path, quote=False) or "`" not in path:
        return None
    parsed = parse_github_url(_url(html), file_link=True)
    if not parsed or _unsafe_path(path):
        return None
    return FileLink(
...
def _url(match: re.Match[str]) -> str:
    return (
        f"https://github.com/{match.group('owner')}/{match.group('repo')}/pull/"
        f"{match.group('pr')}/files#diff-{match.group('anchor')}"
    )


def _unsafe_path(*paths: str) -> bool:
    return any("\r" in path or "\n" in path for path in paths)
...
def validate_file_line(  # noqa: C901
    line: str,
    line_number: int,
    errors: list[str],
    ...
    if not file_link:
        errors.append(
            f"Diff file item {line_number} needs a verified SHA-256 "
            "Files changed anchor"
        )
    else:
    ...
    if file_link and line[file_link.end :] != expected_badges:
        errors.append(
            f"Diff file item {line_number} must contain only its link and "
            "allowed badges"
        )
    operation_kinds = [
        kind
