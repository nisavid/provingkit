...
from .categories import TAXONOMY_NOTE
...
def validate_inventory(
    ...
    for item in items:
        ...
        if not parse_github_url(
            f"https://github.com/{item.repository}/pull/{item.destination_number}"
        ):
            errors.append(f"Stack item #{item.number} has an invalid GitHub URL")
    if position_match:
...
def _validate_position(
    ...
    if current_items and current_items[0] != current_position:
        errors.append(
            f"Stack summary position {current_position} does not match item "
            f"{current_items[0]}"
        )
