...
observed_version = subprocess.check_output(["claude", "--version"], text=True).strip()
if observed_version != "2.1.283 (Claude Code)":
    raise RuntimeError("Claude version drift before preflight")
root = pathlib.Path(sys.argv[1]).resolve() / "native-preflight-v283"
...
manifest = {"kind": "restricted built-in-file-tool preflight, not process sandbox qualification",
            "model": "claude-opus-5-5", "effort": "high", "claude_version": observed_version,
            "catalog": "fresh no-task-data initialize control response on 2.1.283 resolves opus to claude-opus-5-5 and lists high effort",
