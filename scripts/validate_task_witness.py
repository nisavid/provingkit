...
def _project_bridge_controller_to_current(bridge: bytes, current_source: bytes) -> bytes:
    def current_fragment(start: bytes, end: bytes, expected_sha256: str) -> bytes:
        if current_source.count(start) != 1 or current_source.count(end) != 1:
            raise ValueError("Task Witness current controller fragment drift")
...
def validate_bridge_history(root: Path) -> dict[str, object]:
    ...
    if identity_raw != _canonical_json(identity):
        raise ValueError("Task Witness bridge identity is not canonical JSON")
    return {
